...
#
#   Syntax: get_docker_ee_ucp_client_bundle.sh -d docker-ee-ucp-manager
#
#   Pre-requisites:
#
...
function usage () {
    printf 'Usage: get_docker_ee_ucp_client_bundle.sh -h docker-ee-ucp-manager-host\n' >&2
...
if [[ "$HTTP_CODE" != "200" ]]; then
        ...
        printf "https://${DOCKER_EE_HOST}/auth/login\n" >&2
        exit 1
fi  
...
if [[ "$HTTP_CODE" != "200" ]]; then
        ...
        printf "HTTP Code: ${HTTP_CODE}\n" >&2
        printf "https://${DOCKER_EE_HOST}/api/clientbundle\n" >&2
        exit 1
fi        
...
echo "Run the following command to configure your shell for Docker EE commands:"
echo 'eval "$(<env.sh)"'
